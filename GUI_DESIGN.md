...
- Each newly accepted automatic order requires a fresh Start command; the previous
  order's running state and outstanding manual permissions do not carry over.
- Forced quality failures keep the rejection record and are recovered before
  replacement bottles satisfy the order. The simulated recycler services its
  three-cap bin, three-bottle collector and six-bottle-equivalent tank at safe
  mechanism positions. Controllers wait for capacity feedback before completing
  the affected stage, avoiding the former permanent suspension after three rejects.
  Servicing is recorded in Events; recovered bottles lose their active quality
  alerts while retaining their traceability records.
- Pause stops admission and blocks new station permissions; in-progress station
  handshakes finish. Start resumes the retained workpieces. Reset and mode switching
  drain admitted work. This is a supervisory simulation policy, not an
...
The internal `guiStatus` valued signal uses `GuiStatusSender`, a bounded
sample-and-hold sender: socket backpressure never blocks a controller tick.
Unchanged `GuiPermit` values compare equal, preventing repeated permission
transmissions from saturating the signal transport. Swing appends only new log
entries and preserves bottle-table selection during live updates.
...
- `GuiIntegrationTest`: real TCP plus all 30 clock domains; no loading before
  Start, automatic production with recycled rejects/replacements, pause/resume,
...
- `GuiRepeatOrderTest`: three consecutive three-batch orders (90 delivered bottles),
  pause/resume in the second order, painted-bottle versus empty-space clicks,
  reset/draining with work in progress, a new order after reset, and cap-loader
  WORKING/IDLE feedback. Tests use separate ports from the open ABS launch.
- `GuiQualityRecoveryTest`: consecutive 10 + 20 bottle orders with every-third and
  every-fifth failures (30 accepted plus 10 recovered, then 30 accepted plus 6
  recovered), bin/tank servicing, Pause/Resume, Reset with rejects in process,
  and an order after Reset. Every order is checked for no admission before Start.

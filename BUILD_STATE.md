...
> **POS integration update (2026-09-10):** The live POS now replaces `PosStub`.
> Use the existing BuildAll/BuildSystemJ and RunABS/RunSystemJ launch configurations.
> Orders and production feedback use valued SystemJ signals. Batch sequencing,
> recycled-bottle replacements, custom product labels, 0% ingredients, and simulated
> consumable replenishment are implemented. See [README.md](README.md) for the
> current workflow and regression checks. The notes below are the historical
> pre-POS build record; references to hardcoded orders, missing POS/batch boundaries,
> 31 channels, and the stub audit describe that earlier state.
